static char *sccsid = "@(#)lam1.c	34.3 10/24/80";
...
Lterpr()
{
	...
	snpand(0);
	if(lbot==np) protect (nil);
	port = okport(lbot->val,okport(Vpoport->a.clb,stdout));
	putc('\n',port);
	fflush(port);
	return(nil);
}
...
Lprint()
{
	...
	printr(lbot->val,okport(lbot[1].val,okport(Vpoport->a.clb,poport)));
	return(nil);
}
...
Lpatom()
{
	...
	chkrtab(temp);
	port = okport(lbot[1].val, okport(Vpoport->a.clb,stdout));
