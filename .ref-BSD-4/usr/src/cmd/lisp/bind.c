...
static char *sccsid = "@(#)bind.c	34.1 10/3/80";
...
static do_binder()
{
	...
	for(bindage= *pos++; bindage.b_atmlnk!=-1; bindage = *pos++) {
		if( bindage.b_type == 99) {
			...
			/* if(ISNIL(copval(gcload,CNIL)) && loading->a.clb != tatom)
			...
			handy = newfunct();
			handy->bcd.entry = bindage.b_entry;
			handy->bcd.discipline = (bindage.b_type == 0 ? lambda :
								 bindage.b_type == 1 ? nlambda :
								                       macro);

			findstr(bindage.b_atmlnk, array);
			protect(handy);
			mkptr(array)->a.fnbnd = handy;
		}
	}
	initflag = FALSE;
}
