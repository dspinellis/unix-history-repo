...
call  the  function  _g_c_a_f_t_e_r which should be a lambda of one argument.  The
argument passed to _g_c_a_f_t_e_r is the name of the data type which ran  out  and
caused  the garbage collection.  It is _g_c_a_f_t_e_r's responsibility to allocate
more pages of free space.  The default _g_c_a_f_t_e_r makes its decision based  on
the  percentage  of  space  still  in use after the garbage collection.  If
there is a large percentage of space still  in  use,  _g_c_a_f_t_e_r  allocates  a
larger  amount  of  free  space than if only a small percentage of space is
still in use.  The default _g_c_a_f_t_e_r will also print a summary of  the  space
in  use  if  the variable $_g_c_p_r_i_n_t is non nil.  The summary always includes
...
tion is preceded by an asterisk.
     ...
     There are two built-in functions to help you debug your programs: _b_a_k_-
_t_r_a_c_e  and _s_h_o_w_s_t_a_c_k.  When an error occurs (or when you type the interrupt
character), you will be left at a break level with the state of the  compu-
tation frozen in the stack.  At this point, calling  the function _s_h_o_w_s_t_a_c_k
...
running are interpreted or traced, the output of _s_h_o_w_s_t_a_c_k can be very ver-
bose.   The  function  _b_a_k_t_r_a_c_e  prints a summary of what _s_h_o_w_s_t_a_c_k prints.
That is, if showstack would print a list, _b_a_k_t_r_a_c_e  would  only  print  the
first  element  of  the  list.   If  you are running compiled code with the
(_s_t_a_t_u_s _t_r_a_n_s_l_i_n_k) non nil, then fast links are being made.  In this  case,
there  is  not  enough information on the stack for _s_h_o_w_s_t_a_c_k and _b_a_k_t_r_a_c_e.
...
9                                                  Printed: October 21, 1980
     ...
     The top level interpreter for Franz, named _f_r_a_n_z-_t_o_p-_l_e_v_e_l is  defined
in /usr/lib/lisp/toplevel.l It is given control when the lisp system starts
up because the variable top-level is bound to the  symbol  _f_r_a_n_z-_t_o_p-_l_e_v_e_l.
The  first  action  _f_r_a_n_z-_t_o_p-_l_e_v_e_l  takes  is to print out the name of the
...
user-top-level  will  be  _f_u_n_c_a_l_led.   This provides a convenient way for a
...
value of (_s_t_a_t_u_s _i_g_n_o_r_e_e_o_f) is nil, the lisp system will also exit.  Other-
wise  the  end  of  file will be ignored.  When a _r_e_s_e_t is done the current
value of _e_r_r_l_i_s_t is saved away and control is thrown back  up  to  the  top
level where _e_v_a_l is mapped over the saved value of _e_r_r_l_i_s_t.
