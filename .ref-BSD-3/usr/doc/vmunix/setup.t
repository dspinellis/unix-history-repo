...
.I mkfs
above.
(If you have an RM-03 rather than an RP-06 use ``41040'' rather than
...
(write boot block so \fIbproc\fR(8) disk boot scheme will work)
...
.DE
Since all directories created by
.IR tar (1)
will have mode 755 (due to the
.IR umask (1)
above, you should run the commands
.DS
...
\fB#\fR ./mk MODES
.DE
which will reset the permissions of several directories (such as /tmp)
and also correctly set ownerships and modes of ``set-user-id'' programs
(although they were correct already, this does no harm.)
.SH
Making a UNIX boot floppy
.PP
The next thing to do is to make a
.UX
boot floppy, by placing some files on a clean floppy using
.IR arff (1).
Place a clean floppy in the console, and issue the following commands:
.DS
\fB#\fR cd /usr/src/sys/floppy
\fB#\fR arff cr *
.DE
This will put a copy of each file in the directory /usr/src/sys/floppy
onto the floppy.
...
.PP
The boot floppy you created does not have enough files on
it to deadstart the machine with.  If your standard console floppy does
not have a multi-segment RT-11 directory structure on it, and if you make
a copy of it using
.IR flcopy (1),
delete unneeded files to make space for the
.UX
boot programs, and then add the files with
.DS
\fB#\fR cd /usr/src/sys/floppy
\fB#\fR arff r *
.DE
you should be able to cold-start the machine from this floppy.
The following set of files is believed to be adequate to cold-start
the machine:
.TS
center;
l l l.
boot.exe	restar.cmd	wcsmon.sys
consol.sys	restar.ilv	wcssrv.bin
filea.pat	vmb.exe
pcs.pat	wcs\fInnn\fR.pat
.TE
It is also useful to have the console help files on your floppy;
their names end in ``.hlp''.
.SH
...
.PP
Note that /usr/src/cmd/init.vm.c will have to be recompiled if there are to be
more than 100 terminals.
Also note that if the special file is inaccessible when \fIinit.vm\fR tries to create a process
...
start dumps if one is needed.\(dg
.FS
