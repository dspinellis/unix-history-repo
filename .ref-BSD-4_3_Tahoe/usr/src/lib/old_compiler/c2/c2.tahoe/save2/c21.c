...
bmove() {
	...
	if (debug) {
		...
		for (r=0; r<NUSE; r++) if (isused(r))
			printf("r%d: %s (%d times)\n",
			    r, uses[r] && uses[r]->code? uses[r]->code:"",
			    usecnt[r]);
		printf("-\n");
	}
	r=(p->subop>>4)&0xF;
	...
	case JBR: case JMP:
		...
		if ((p->subop&0xF)==RET) {
			switch((p->subop>>4)&0xF) {
				case 2: adduse(1,p); regs[1][0]= -1;
				case 1: adduse(0,p); regs[0][0]= -1;
			}
			break;
		}
	...
	register struct node *p;
{
	...
	if (tempreg(lastrand,r) ||
	    (has2ops(p) && tempreg(regs[RT2],r) && !isused(r))) {
		/*
		...
		default:
			 ...
			 */
			if (!isused(r) && ((dblflg&1)==0 || !isused(r+1))) {
	...
	/*
	 * Clobber any registers written, but
	 * don't clobber use/reference count.
	 */
	if ((r=flow) >= 0) {
		olduse=uses[r]; deluse(r);
		*(short *)(regs[r])=0;
		/* if r0 destroyed, dont keep r1 */
		if (dblflg&1) {
			olduse1=uses[++r]; deluse(r);
			*(short *)(regs[r])=0;
		}
	}
	...
	if (*cp1++=='$' && isstatic(cp1)) {
		if (p->op==MOV && p->subop==LONG) {
			...
			if (uses[r]==0) {adduse(r,p); regs[r][0]=OPX<<4;}
			p->pop=0;
		}
	}
#endif MOVAFASTER
