...
and  they will not be reclaimed unless they are _c_l_o_s_ed.  All
...
value of the symbol _p_i_p_o_r_t and if output is being done  then
the  default port is the value of the symbol _p_o_p_o_r_t.  Furth-
...
output will also go to the port _p_t_p_o_r_t  if  it  is  a  valid
port.   Output  destined  for  the  standard output will not
reach the standard output  if  the  symbol  ^_w  is  non  nil
