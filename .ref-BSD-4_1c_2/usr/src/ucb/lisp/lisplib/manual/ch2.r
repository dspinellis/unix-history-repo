...
2.1.  Lists
     ...
     The following functions  exist  for  the  creation  and
manipulating  of lists.  Lists are composed of a linked list
of objects called either 'list cells', 'cons cells' or 'dtpr
cells'.  Lists are normally terminated with the special sym-
bol nil.  nil is both a symbol and a representation for  the
empty list ().
...
(quote! g_form1 ...)
     ...
     RETURNS: The list  resulting  from  the   splicing  and
              insertion  process described below on the list
              beginning with g_form1.
     ...
     NOTE: _q_u_o_t_e!  is the complement of the  _l_i_s_t  function.
           ...
           !!. ! g_form evaluates g_form and  the  value  is
           inserted  in  the  place  of  the call; !! g_form
...
Data Structure Access                                    2-3
           ...
           evaluates g_form and the value  is  spliced  into
           the  place  of  the call.  Splicing in means that
           the parentheses surrounding the list are  removed
           as  the example below shows.  Use of the evaluate
           operators can occur at any level in a form  argu-
           ment.
...
(tailp 'l_x 'l_y)
     ...
     RETURNS: t iff by _c_d_ring down l_y zero or more times, a
              list cell _e_q to l_x is found.
...
(remove 'g_x 'l_l)
     ...
     RETURNS: a _c_o_p_y of l_l  with  all  top  level  elements
              _e_q_u_a_l to g_x removed.
...
(dreverse 'l_arg)
     ...
     NOTE: The functions _n_r_e_v_e_r_s_e and _d_r_e_v_e_r_s_e do the rever-
           sal in place, that is the list structure is modi-
           fied.
...
(memq 'g_arg1 'l_arg2)
     ...
     2.3.  Symbols and Strings
          ...
          In many of the following functions the distinction
     between  symbols  and  strings is somewhat blurred. The
     difference is that a symbol is a  lisp  object  with  a
     value,  property  list, function binding and a external
     representation (print  name)  which  happens  to  be  a
     string.   A string is a simply a constant, the value of
     a string is the same string.  If a symbol is  given  to
     one  of  the  string  manipulation functions below, its
...
(substringn 'st_string 'x_index ['x_length])
     ...
     NOTE: If x_length is  not  given,  the  maximum  length
           string  is  returned.  If x_index is negative the
           string begins at the x_index_t_h character from the
           end.   If  x_index  is  out  of  bounds,  nil  is
           returned.
...
(exploden 'g_arg)
    ...
    ____________________________________________________
     ...
     array compatibility package represent only  one  simple
     way of using the array structure of FRANZ LISP.
9
...
(fillarray 's_array 'l_itms)
     ...
     2.5.  Hunks
          ...
          Hunks are vector like objects whose size can range
     from 1 to 128 elements.  Internally hunks are allocated
     in sizes which are powers of 2.   In  order  to  create
     hunks  of  a given size, a hunk with at least that many
     elements is allocated and a distinguished symbol  EMPTY
     is  placed  in those elements not requested.  Most hunk
     functions  respect  those  distinguished  symbols,  but
     there   are  two  (*_m_a_k_h_u_n_k  and  *_r_p_l_a_c_x)  which  will
     overwrite the distinguished symbol.
...
(putdisc 'y_func 's_discipline)
     ...
     2.7.1.  assoc list
          ...
          An `assoc list' (or alist) is a common  lisp  data
...
(sublis 'l_alst 'l_exp)
     ...
     WHERE:   l_alst is an assoc list, that is  it  has  the
              form
              ((_k_e_y_1 . _v_a_l_1) (_k_e_y_2 . _v_a_l_2) ...)
     ...
     sequence  of keys and values.  Normally a property list
     is stored on a symbol. A list is a  'disembodied'  pro-
     perty  list  if  it contains an odd number of elements,
...
Data Structure Access                                   2-30
    ...
    ____________________________________________________
     ...
     2.7.3.  tconc structure

          A tconc  structure  is  a  special  type  of  list
     designed to make it easy to add objects to the end.  It
     consists of a list cell whose _c_a_r points to a  list  of
     the  elements  added  with _t_c_o_n_c or _l_c_o_n_c and whose _c_d_r
     points to the last list cell of the list pointed to  by
     the _c_a_r.
...
Data Structure Access                                   2-31
    ...
    ____________________________________________________
     ...
     2.8.  Random functions

          The following functions don't fall into any of the
