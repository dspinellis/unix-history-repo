...
char *procname;
   ...
   if (flags == 0) {                    /* undeclared */
      ...
      else if ((p.bp = blocate(id)) != NULL) {	/* check builtin */
         lp->l_flag = F_BUILTIN;
         lp->l_val.global = putglob(id, F_BUILTIN | F_PROC, -1, 0);
         }
      else {					/* implicit local */
	 if (imperror)
...
struct centry *putconst(n, flags, len, val)
int n;
int flags, len;
union {
   long  ival;
   ...
   if (flags & F_INTLIT) {
      ...
      if (val.ival < (long)(int)MINSHORT | val.ival > (long)(int)MAXSHORT)
	 p->c_flag |= F_LONGLIT;
      }
