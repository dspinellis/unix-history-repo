1
                     FEDERAL SOFTWARE TESTING CENTER
                    FORTRAN COMPILER VALIDATION SYSTEM
                      VERSION 2.0  82/08/02*18.33.46
 
 *FM906BEGIN*            TEST RESULTS - FM906

         TEST DATE*TIME= 07-Nov-85          -  COMPILER= CCI 5.2             
 
 LSTDI2 - (372)  LIST DIRECTED INPUT FOR D.P. AND COMPLEX DATA TYPES

 ANS REF. - 13.6  12.4
 
 
  TEST   PASS/FAIL       DISPLAYED RESULTS       REMARKS
 -------------------------------------------------------------------------------
                                                 THIS PROGRAM HAS  28 TESTS

     1     PASS  
     2     PASS  
     3     PASS  
     4     PASS  
     5     PASS  
     6     PASS  
     7     PASS  
     8     PASS  
     9     PASS  
    10     PASS  
    11     PASS  
    12     PASS  
1
  TEST   PASS/FAIL       DISPLAYED RESULTS       REMARKS
 -------------------------------------------------------------------------------
    13     PASS  
    14     PASS  
    15     PASS  
    16     PASS  
    17     PASS  
    18     PASS  
    19     PASS  
    20     PASS  
    21     PASS  
    22     PASS  
    23     PASS  
    24     PASS  
    25     PASS  
    26     PASS  
    27     PASS  
    28     PASS  
 
 -------------------------------------------------------------------------------
 
                         28 TESTS PASSED
                          0 TESTS FAILED
                          0 TESTS DELETED
                          0 TESTS REQUIRE INSPECTION
                         28 OF  28 TESTS EXECUTED
 
 *FM906END*              END OF TEST - FM906

 TC-85-   -410                                    *             /850703       
 FOR OFFICIAL USE ONLY                                        COPYRIGHT  1982
