     ...
     register tree parent, binfo;
{
  ...
  while (1)
	...
	break;
      ...
      is_private = (HOST_WIDE_INT)search_stack->first[head++];
...
/* Return the number of levels between type PARENT and the type given
   ...
   The term `leftmost' should be defined in g++int.texi and the
   significance should be explained.  If it is, this message should be
   deleted, if not, and you understand the term, please add
   documentation for it in g++int.texi.  Also, the part about the
   ``PARENT is its own main type variant'' should give reasons when
   and why this happens, and why it is important, and a better
   explaination of what this routine does both when that is true and
   when it is not.
     ...
     tree *path_ptr;
{
  ...
  while (1)
	...
	break;
      ...
      binfo = search_stack->first[head++];
      depth = (HOST_WIDE_INT)search_stack->first[head++] + 1;
      is_private = (HOST_WIDE_INT)search_stack->first[head++];
      if (path_ptr)
	{
	...
	if (tp < search_tail)
	  {
		...
		if (rval = recursive_bounded_basetype_p (binfo, base_binfo, rval, 1))
		  break;
	      }
	    if (rval > 0)
...
/* Look for a field named NAME in an inheritance lattice dominated by
   ...
   return TYPE_DECLs, if no TYPE_DECL can be found return NULL.  */
tree
	  ...
	  if (fn == dfs_init_vbase_pointers)
	    {
		 ...
		 is that when we hit the first virtual base class, we must
		...
		= vbase_decl_ptr_intermediate;
	      int save_saw_first_vbase = saw_first_vbase;

	      if (!saw_first_vbase && TREE_VIA_VIRTUAL (base_binfo))
		{
		  saw_first_vbase = 1;
