	...
	register NUMBER *q1, *q2;
{
	...
	if (iszero(n1) || iszero(n2))
		return qlink(&_qzero_);
	if (!isunit(n1) && !isunit(d2)) {	/* possibly reduce */
		zgcd(n1, d2, &tmp);
		if (!isunit(tmp)) {
			zquo(q1->num, tmp, &n1);
			zquo(q2->den, tmp, &d2);
	...
	if (!isunit(n2) && !isunit(d1)) {	/* again possibly reduce */
		zgcd(n2, d1, &tmp);
		if (!isunit(tmp)) {
			zquo(q2->num, tmp, &n2);
			zquo(q1->den, tmp, &d1);
	...
	register NUMBER *q1, *q2;
{
	...
	if (isunit(q1->num))
		num = q2->den;
	else if (isunit(q2->den))
		num = q1->num;
	else
		zmul(q1->num, q2->den, &num);
	if (isunit(q1->den))
		den = q2->num;
	else if (isunit(q2->num))
		den = q1->den;
	else
	...
	long pow;
{
	...
	if ((pow > 1000000L) || (pow < -1000000L))
		error("Very large scale value");
	numshift = isodd(q->num) ? 0 : zlowbit(q->num);
	denshift = isodd(q->den) ? 0 : zlowbit(q->den);
	if (pow > 0) {
		tmp = pow;
		if (tmp > denshift)
		tmp = denshift;
	...
	NUMBER *q1, *q2;
{
	...
	if (qisfrac(q1) || qisfrac(q2))
		error("Non-integers for logical or");
	if ((q1 == q2) || qiszero(q2))
		return qlink(q1);
	if (qiszero(q1))
		return qlink(q2);
	...
	NUMBER *q1, *q2;
{
	...
	if (qisfrac(q1) || qisfrac(q2))
		error("Non-integers for logical and");
	if (q1 == q2)
		return qlink(q1);
	if (qiszero(q1) || qiszero(q2))
		return qlink(&_qzero_);
	...
	NUMBER *q1, *q2;
{
	...
	if (qisfrac(q1) || qisfrac(q2))
		error("Non-integers for logical xor");
	if (q1 == q2)
		return qlink(&_qzero_);
	if (qiszero(q1))
		return qlink(q2);
	...
	long n;
{
	...
	h2[0] = (n & BASE1);
	h2[1] = (n >> BASEB);
	if (isunit(q->den))	/* integer compare if no denominator */
