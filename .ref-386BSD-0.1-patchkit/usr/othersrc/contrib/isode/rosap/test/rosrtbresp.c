...
char	**argv, **envp;
{
    ...
    if (RtBInit(argc, argv, rts, rti) == NOTOK)
        fprintf(dfp, "initialisation fails: %s",
    ...
    if (RtBeginResponse(sd, RTS_ACCEPT, NULLPE, rti) == NOTOK)
        fprintf(dfp, "RT-OPEN.RESPONSE: %s",
            RtErrString(rti->rti_abort.rta_reason));
   
    if (RoSetService (sd, RoRtService, &rois) == NOTOK)
	fprintf(dfp, "RoSetService: %s", RoErrString (rop -> rop_reason));
...
struct AcSAPfinish *acf;
{
	...
	fprintf(dfp, "RtWaitRequest:RT-CLOSE.RESPONSE: %s\n",
...
struct RoSAPend *roe;
{
    ...
    if (RtEndResponse(sd, &rtis) == NOTOK) {
        fprintf(dfp, "RT-END.RESPONSE: failed: %s\n",
            RtErrString(rtis.rti_abort.rta_reason));
        exit(6);
    }
