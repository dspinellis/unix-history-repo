...
char	**argv, **envp;
{
	...
	if (acs->acs_info[0] == NULLPE) {
	    fprintf(stderr, "A-ASSOCIATE.RESPONSE: NULL User data\n");
	    exit(1);
	}
	if ((servicetype = num2prim(acs->acs_info[0])) == NOTOK) {
	    if (acs->acs_info[0]->pe_errno != PE_ERR_NONE) {
		fprintf(stderr, "A-ASSOCIATE.RESPONSE: Bad User data\n");
		exit(1);
	    }
	}
    }
