     ...
     tree decl;
{
  ...
  if (context_stack)
    ...
    {
      cadillac_printf ("start top-level decl", IDENTIFIER_POINTER (DECL_NAME (decl)));
     ...
     tree decl;
{
  ...
  if (context_stack)
    ...
    {
      cadillac_printf ("end top-level decl", IDENTIFIER_POINTER (DECL_NAME (decl)));
      CWriteHeader (conn, EndDefMType, 0);
      CWriteLength (conn);
