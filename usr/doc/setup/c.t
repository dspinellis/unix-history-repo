...
which might be encountered while trying to install the 4.2BSD
...
the release consult this section for an indication of the
problem before contacting our group.
.SH
\fBUsing the distribution console medium.\fP
.LP
This section describes problems which may occur when using
...
If a hard i/o error occurred while reading a floppy, try reseting
the console LSI-11 by powering it on and off.  If you can not
boot the cassette's bootstrap monitor, verify the standard
DEC console cassette can be read; if it can not, your cassette
is broken \- not uncommon.
.LP
...
You have requested
.I format
to work on a device for which it has no driver, or which does not exist;
only the indicated devices are supported.
.LP
...
is the case and you are booting off the mini-root file
...
to boot the true root file system, try booting off the
...
.LP
The file header for the program indicated a size larger
...
.LP
This occured because the system was unable to locate
the program /etc/init.  
...
to run is corrupted, or simply missing /etc/init.  Try
reinstalling the appropriate file system or installing a
version of init.
.LP
...
the system on the ``hk'', then try and boot the system 
