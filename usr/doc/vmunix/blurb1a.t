...
Dear Colleague:
.PP
We are happy to announce the availability of a revision of the Fourth
...
.FE
licenses. The new distribution is called 4.1bsd and supersedes the
...
information to the many sites which wish to run 4.1bsd, we have
produced five documents which answer in great detail many of the
...
.IP \(bu
.B
Proposals for enhancements of UNIX on the VAX
.R
describes several proposals for enhancing UNIX on the VAX to meet the
needs of the users.
.IP \(bu
.B
Performances Issues of VMUNIX Revisited,
Comments on the performances of UNIX on the VAX,
and
UNIX and VMS, Some Performance Comparisons
.R
provide insight into the issue of UNIX versus VMS performance.
.RE
.SH
...
and Western Electric which authorizes you to have UNIX/32V.  Note that
this must be a complete copy of the agreement, not, for example, merely
the signature page or the cover page.  Since our agreement with you
will allow you to use our software on all CPU's which you have properly
licensed for UNIX/32V, we do not require you to document that any
particular CPU is licensed.
.IP \(bu
...
UNIX/32V agreement.  One of the copies will be returned to you after it
has been executed on behalf of the Regents.
.IP \(bu
A check for $300 U.S. payable to ``the Regents of the University of
...
have one with them, you must have one with us.
.PP
.B
DARPA Sites.
.R
...
(2) a Site Information Form, and (3) a check or IOC for $300.  Forward
these items to us through Howard Symonds, Materiel Manager, 2543
Channing Way, Berkeley, CA 94720.  Mr. Symonds will verify the status
of your UNIX/32V license for us.
.PP
If you have questions about the distribution or the licensing process,
contact the Jeri Kotani at the above address, phone
