...
caused the garbage collection is preceded by an asterisk.
     ...
     There are two built-in functions to help you debug your
...
look  at  stack frames within lisp.  The programs which take
advantage of these  things  are  in  /usr/lib/lisp  and  are
called step and fixit.  They are maintained by the people at
Carnegie-Mellon (currently Lars Ericson is in charge of  the
code  there).   There are documentation files for these pro-
grams in /usr/lib/lisp as well.  We run compiled lisp almost
exclusively  at  Berkeley.   Sites which run a mostly inter-
preted code should examine these files.
...
9                                       Printed: July 9, 1981
     ...
     The top level interpreter for Franz,  named  _f_r_a_n_z-_t_o_p-
_l_e_v_e_l  is  defined  in  /usr/lib/lisp/toplevel.l It is given
control when the lisp system starts up because the  variable
top-level is bound to the symbol _f_r_a_n_z-_t_o_p-_l_e_v_e_l.  The first
action _f_r_a_n_z-_t_o_p-_l_e_v_e_l takes is to print out the name of the
current  version of the lisp system.  Then it loads the file
.lisprc from the HOME directory of the person  invoking  the
lisp  system  if  that file exists.  The .lisprc file allows
you to set up your own defaults, read in files, set up auto-
loading   or anything else you might want to do to personal-
ize the lisp system.   Next,  the  top  level  goes  into  a
prompt-read-eval-print  loop.   Each  time  around the loop,
