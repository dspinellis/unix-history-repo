...
addsob()
{
	...
	if (p->combop==T(DEC,LONG) && p1->op==CBR) {
		...
		if (p1->combop==T(CMP,LONG) && (p2=p1->forw)->op==CBR) {
			...
			if (abs(p->seq - p2->ref->seq)<8) {/* inside byte displ range */
				if (p2->subop!=JLE) continue;
				/*
				 * acbl take twice as long as aobleq
				 * so changed to aobleq - dm 1/19/82
				 */
				p->combop=AOBLEQ;
			...
			if (abs(p->seq - p2->ref->seq)>8) {/* outside byte displ range */
				...
				p->combop=T(ACB,LONG);
				cp2=regs[RT1]; cp1=regs[RT2]; while (*cp2++= *cp1++); /* limit */
				cp2=regs[RT2]; cp1="$1"; while (*cp2++= *cp1++);
				cp2=regs[RT3]; cp1=p->code; while (*cp2++= *cp1++);
				p->pop=0; newcode(p);
				p->labno = p2->labno; delnode(p2); delnode(p1); nsob++;
