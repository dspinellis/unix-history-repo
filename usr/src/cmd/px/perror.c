...
static char sccsid[] = "@(#)perror.c 4.1 10/10/80";
...
*/
	...
	case EBADFNUM:
		...
		break;
	case ENUMNTFD:
		fputs("Unknown name found on enumerated type read\n",stderr);
		break;
	case ENAMRNG:
		fputs("Enumerated type value out of range on output\n",stderr);
		break;
	case EFMTSIZE:
		fputs("Negative format width\n",stderr);
		break;
	case ENARGS:
		fputs("Improper number of arguments to formal routine\n"
			,stderr);
		break;
	case EGOTO:
	...
	case EOUTOFMEM:
		...
		break;
	case ETRASHHEAP:
		fputs("Improper use of new variables has caused heap integrity check to fail\n",stderr);
		break;
	case ECTLWR:
		fputs("Range lower bound out of set bounds\n", stderr);
		break;
	case ECTUPR:
		fputs("Range upper bound out of set bounds\n", stderr);
		break;
	case ECTSNG:
		fputs("Single value out of set bounds\n", stderr);
		break;
	case EARGV:
