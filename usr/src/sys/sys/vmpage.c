...
valid:
	...
	if (pte->pg_fod == 0) {
		fileno = -1;
		bnswap = bncache = bn = dbtofsb(vtod(p, v, &u.u_dmap, &u.u_smap));
		dev = swapdev;
	} else {
		fileno = ((struct fpte *)pte)->pg_fileno;
		bn = ((struct fpte *)pte)->pg_blkno;
		bnswap = dbtofsb(vtod(p, v, &u.u_dmap, &u.u_smap));
		if (fileno > PG_FMAX)
	...
	if (opte.pg_fod) {
		...
		if (fileno == PG_FZERO || fileno == PG_FTEXT) {
			...
			/*
			 * Vreading block... update summary information.
			 */
			if (u.u_ofile[fileno] == NULL)
				panic("pagein u.u_ofile");
		 ...
		 */
		v = kluster(p, v, pte, B_READ, &klsize, klin, bn);
		/* THIS COULD BE COMPUTED INCREMENTALLY... */
		bncache = bn = dbtofsb(vtod(p, v, &u.u_dmap, &u.u_smap));
		pte->pg_vreadm = opte.pg_vreadm;
	}

	distcl(pte);
	swap(p, fsbtodb(bn), ptob(v), klsize * ctob(CLSIZE),
	    B_READ, B_PGIN, dev, 0); 
...
skipswap:
	...
	for (i = 0; i < klsize; i++) {
		...
		c->c_intrans = 0;
		if (type == CTEXT && c->c_blkno == 0 && bncache) {
			mhash(c, dev, bncache);
			bncache++;
		}
...
top:
		...
		if (pte->pg_v) {
			...
			if ((rp->p_flag & (SANOM|SUANOM)) || fifo)
				goto take;
		} else {
take:
			if (c->c_type != CTEXT) {
