...
int	ctlr, drive, op_mask;
{
	int	tokens[20];
	char	line[132];
	int	count, savedevflags = 0;
	register struct disklabel *lp;
	struct disklabel *getdiskbyname(), *promptfordisk();
	...
	if (kill_processes == true)
		return;
	if (lp->d_typename[0] != 0 && strcmp(lp->d_typename, line) == 0)
		return;
	lp = 0;
	...
	for (; count < ndrives; count++)
		if (strcmp(vdproto[count].d_typename, line) == 0)
			break;
	if (count < ndrives)
		lp = &vdproto[count];
	while (lp == 0)
...
check:
	while (op_mask & FORMAT_OP && lp->d_traksize == 0) {
		...
		if (kill_processes == true)
			return;
		if (line[0] == 0 || (lp->d_traksize = atol(line)) == 0)
