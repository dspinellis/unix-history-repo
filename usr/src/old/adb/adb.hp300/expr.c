...
{	/* item | monadic item | (expr) | */
	...
	switch ((int)readchar()) {
		    ...
		    case '*':
			term(a|1); expv=lchkget(expv,DSP); return(1);

		    case '@':
			term(a|1); expv=lchkget(expv,ISP); return(1);
...
{	/* name [ . local ] | number | . | ^ | <var | <register | 'x | | */
	...
	THEN	readsym();
		IF lastc=='.'
		THEN	frame= *(ADDR *)(((ADDR)&u)+FP-radj); lastframe=0;
			callpc= *(ADDR *)(((ADDR)&u)+PC-radj);
			WHILE errflg==0
			DO  savpc=callpc;
				findsym(callpc,ISYM);
			...
			FI
		ELIF (symp=lookup(isymbol))==0
		THEN	hexp=isymbol;
			lastc='#';
			IF !getnum(hexsym) ORF *hexp
			THEN	error(BADSYM);
			FI
	...
	THEN	savc=rdc();
		...
		THEN	IF kcore THEN expv = *(int *)regptr;
			ELSE	IF regptr >= sizeof (struct user)
				THEN	regptr -= radj;
				FI
				expv= * (ADDR *)(((ADDR)&u)+regptr);
			FI
...
{
	lastc=0;
	IF hexp >= isymbol ANDF hexp < &isymbol[sizeof isymbol] ANDF *hexp
	THEN	return (lastc= *hexp++);
	FI
	return (0);
}
