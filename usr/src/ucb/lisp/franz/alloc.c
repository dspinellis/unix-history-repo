#ifndef lint
static char *rcsid =
   "$Header: /na/franz/franz/RCS/alloc.c,v 1.3 83/03/03 12:11:25 jkf Exp $";
#endif

/*					-[Thu Mar  3 11:49:20 1983 by jkf]-
 * 	alloc.c				$Locker:  $
...
struct types *type_struct;
{
	...
	if( (int) datalim >= TTSIZE*NBPG+OFFSET ) return(2);
...
register struct types *typestr;
{
 ...
 again:
    ...
    /* must make sure that if the vector we are allocating doesnt
       ...
       the size left over */
    if((bytes & (NBPG - 1)) > (NBPG - 2*sizeof(long))) bytes += NBPG;
    bytes = roundup(bytes,NBPG);
...
{   register lispval aptr;
	...
	aptr = (lispval) newatom(TRUE); /*share print name of atoms on oblist*/
...
LImemory()
{
	...
	printf("  hole free: %d bytes = %d pages\n\n",
	       holend-curhbeg, (holend-curhbeg)/NBPG);
#endif 
    nextadr = (int) xsbrk(0);	/* next space to be allocated */
    pagesinuse = nextadr/NBPG;
