#ifndef lint
static char *rcsid =
   "$Header: /na/franz/franz/RCS/eval.c,v 1.2 83/01/29 12:32:46 jkf Exp $";
#endif

/*					-[Sat Jan 29 12:19:37 1983 by jkf]-
 * 	eval.c				$Locker:  $
 * evaluator
 *
...
/* Ifclosure :: evaluate a fclosure
 * the argument clos is a vector whose property is the atom fclosure
 * the form of the vector is
 *   0: function to run
 * then for each symbol there are 3 vector entries
 *   1: symbol
 * either
 *   2: nil
 *   3: value to bind symbol to
 * or
 *   2: a vector (another fclosure)
 *   3: fixnum which is the index into the fclosure of the
 *      slot containing the value
 *  .... and so on ...
 *
 * when entering a fclosure, the values of the globals are set to the
 * saved values in the fclosure.
 * When exiting a fclosure, the current values of those globals are stored
 * back in.
 * note: we don't do an unwind-protect to insure that we restore the values.
 *  perhaps we should, it isn't clear though.
 *
...
register lispval clos;
{
    ...
    /* vector length must be positive and 1+ a multiple of 3 */
    if ((vlength < 1) || ((vlength-1) % 3) != 0)
	errorh1(Vermisc,"funcall: fclosure has wrong size ",nil,FALSE,0, clos);

    numvars = (vlength - 1) / 3;
    
    for (i = 1 ; i < vlength ; i += 3)
    {
	atm = clos->v.vector[i];
	if((vect=clos->v.vector[i+1]) != nil)
	{
	    /* this is a pointer to another fclosure */
	    vect = vect->v.vector[clos->v.vector[i+2]->i];
	}
	else vect = clos->v.vector[i+2];
	PUSHDOWN(atm,vect);
    }
    ...
    /* this should never be an error (famous last words) */
    if((bnp-oldbnp) != numvars)
        errorh1(Vermisc,"funcall: incorrect number of values after fclosure:",
		  nil,FALSE,0,clos);

    /* pop off current values and store current values in fclosure */
    for(i = vlength - 3 ; i > 0 ; i -= 3)
    {
       --bnp;
       if((vect = clos->v.vector[i+1]) != nil)
       {
	   /* store value in another fclosure */
	   vect->v.vector[clos->v.vector[i+2]->i] = bnp->atm->a.clb;
       }
       else clos->v.vector[i+2] = bnp->atm->a.clb;
       bnp->atm->a.clb = bnp->val;
    }
