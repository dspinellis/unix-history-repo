...
#ifndef lint
static char sccsid[] = "@(#)getinfo.c	5.3 (Berkeley) 3/31/86";
#endif not lint
...
static int
GetAnswer(nsAddrPtr, queryType, msg, msglen, iquery, hostPtr)
	...
	register HostInfo 	*hostPtr;
{
	...
	if (headerPtr->rcode != NOERROR) {
		...
		"Failed: %s, num. answers = %d, ns = %d, additional = %d\n", 
			rcodes[headerPtr->rcode], ancount, nscount, arcount);
	    }
	...
	/*
	 * At this point, only non-authoritative answers for
	 * the T_A query type remain. For other query types,
	 * additional information might be found in the additional
	 * resource records part.
	 */

	cp = res_skip((char *) &answer, 2);
...
int
GetHostInfo(nsAddrPtr, queryType, name, hostPtr)
	struct in_addr *nsAddrPtr;
	int 		queryType;
	char 		*name;
	HostInfo 	*hostPtr;
{
	...
	n = res_mkquery(QUERY, name, C_ANY, queryType,
	...
	result = GetAnswer(nsAddrPtr, queryType, (char *) &buf, n, 0, hostPtr);
	...
	HostInfo 	*hostPtr;
{
	...
	return(GetAnswer(nsAddrPtr, T_A, (char *) &buf, n, 1, hostPtr));
}

/*
 *******************************************************************************
	...
	HostInfo 	*hostPtr;
{
	...
	return(GetAnswer( nsAddrPtr, queryType, (char *) &buf, n, 0, hostPtr));
}
#endif  notdef
