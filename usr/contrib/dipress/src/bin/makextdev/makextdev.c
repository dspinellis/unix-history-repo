...

{
    ...
    read(descfd, &device_entry, sizeof(device_entry));

    /* skip over the point size table */
    lseek(descfd, (device_entry.num_sizes + 1) * sizeof(short), 1);

    /* read char_index_table and char_name arrays */
    read(descfd, char_index_table, device_entry.spec_char_num * sizeof(short));
    read(descfd, char_name, device_entry.spec_name_len);
...

{
    ...
    read(outfile, &font_entry, sizeof(font_entry));
    lseek(outfile, (unsigned char)(font_entry.num_char_wid) * 3, 1);	/* skip width, kern, and code */
    read(outfile, font_index_table, device_entry.spec_char_num + 128 - 32);
    close(outfile);
...
*/
    ...
    write(outfile, extend_table, (device_entry.spec_char_num + 128-32) * sizeof(short));
    close(outfile);
    fclose(fontfile);
}
