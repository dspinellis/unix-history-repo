...
#ifdef SCCSID
static char	*SccsId = "@(#)header.c	2.41	3/19/86";
#endif /* SCCSID */
...
genversion()
{
	...
	if (my_version == NULL) {
		...
		if (strcmp(LOCALSYSNAME, FULLSYSNAME))
			(void) sprintf(rb, "version %s; site %s.%s%s",
				news_version, LOCALSYSNAME, FULLSYSNAME, MYDOMAIN);
		else
#endif /* !HIDDENNET */
			(void) sprintf(rb, "version %s; site %s%s",
				news_version, FULLSYSNAME, MYDOMAIN);
		while (t = index(rb, '\t'))
			*t = ' ';
		my_version = malloc((unsigned)strlen(rb) + 1);
		if (my_version == NULL)
			xerror("malloc failed for my_version");
		(void) strcpy(my_version, rb);
	}
	return my_version;
}
