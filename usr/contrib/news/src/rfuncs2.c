...
#ifdef SCCSID
static char	*SccsId = "@(#)rfuncs2.c	1.27	6/7/85";
#endif /* SCCSID */
...
int	notauthor;
{
	...
	if (pid > 0)
		return 0;
	if (notauthor)
		(void) sprintf(bfr, "%s/%s -t 'cmsg cancel %s' -n %s -d local < /dev/null",
		    LIB, "inews", hp->ident, hp->nbuf);
	else {
		if (hp->distribution[0] == '\0')
			(void) sprintf(bfr, "%s/%s -t 'cmsg cancel %s' -n %s < /dev/null",
			    LIB, "inews", hp->ident, hp->nbuf);
		else
			(void) sprintf(bfr, "%s/%s -t 'cmsg cancel %s' -n %s -d %s < /dev/null",
