	...
	dev_t dev;
{
	 ...
	 * and low ender machines are to communicate.
	 */
...
#endif	SWABALL
		bcopy(en, mtod(mtop, struct en_header *),
...
#ifdef	SWABALL
		/*
		 * enstart doesn't swab header fields; en_type is in
		 * net order and interface expects host order.
		 */
		en->en_type = ntohs(en->en_type);
#endif	SWABALL
		goto gotheader;
...
#endif
#if	NENETFILTER > 0
gotheader:	/* will add en_shost later, in enstart() */
#endif
	/*
...
#if defined(SWABALL) || defined(ENF_SWABIPS)
/*
 * Swab bytes
