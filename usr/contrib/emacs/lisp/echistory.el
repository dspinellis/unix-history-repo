		...
		(save-window-excursion
		  ...
		  (let ((mode-line-format "-- %[%m%]  %M  ----%3p-%-"))
			...
			(progn (ding)
		      ...
		      (let ((Helper-mode-name "Electric History")
			    (Helper-major-mode 'electric-command-history)
			    (Helper-return-blurb "return to History"))
			(Electric-command-loop 'electric-history-quit "->" t)))))))
      (set-buffer "*Command History*")
