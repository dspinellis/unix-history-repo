...
#ifdef SCCSID
static char	*SccsId = "@(#)expire.c	2.43	3/19/86";
#endif /* SCCSID */
...
char	**argv;
{
	...
	while (TRUE) {
		...
		if (nohistory) {
			...
			fp = access(filename, 04) ? NULL : fopen(filename, "r");
		} else {
			char dc;
			if (fgets(afline, BUFLEN, ohfd) == NULL)
				break;
			...
			*p2++ = '\t';
			...
			if (!ngmatch(nbuf, ngpat) ||
			     ((rectime+expincr > now) && !dorebuild && !frflag
				&& !usepost && recdate[0] != ' '))
				goto keephist;
			if (recdate[0] != ' ') {
				grpsleft[0] = '\0';
...
saveit:
		...
		if (!h.expdate[0] || ignorexp == 2 ||
			...
			exptime = newtime;
		if (frflag ? strcmp(baduser,h.from)==0 : now >= exptime) {
nailit:
#ifdef DEBUG
			printf("cancel %s\n", filename);
#else /* !DEBUG */
...
checkdate:
		if (grpsleft[0] == '\0' && now >= rectime + dropincr) {
			if (verbose > 3)
