...
/* $Header: XOpenDisplay.c,v 10.8 86/02/01 15:37:30 tony Rel $ */
	...
	register char *display;
{
	...
	char buf[3];			/* place to form display string */
	...
	if (display == NULL || *display == '\0') {
	    	char *disp;
		/* No display given check the DISPLAY environment variable. */
		if ((disp = getenv("DISPLAY")) == NULL) {
			/* Oops! No DISPLAY environment variable - error. */
			return(NULL);
		}
		strncpy(displaybuf, disp, sizeof(displaybuf));
	}
	else {
	...
	*(displayptr++) = '\0';
	...
	if (strcmp("unix", displaybuf) == 0) {
		...
		bcopy((char *)host_ptr->h_addr, &inaddr.sin_addr, sizeof(inaddr.sin_addr));
	...
	req->param.u[2] = BlackPixel;
	GetReq(X_MakePixmap, 0);
	req->param.l[0] = 0;
	req->param.u[2] = WhitePixel;
	...
	dpy->vnumber = rep.param.s[2];	/* X protocol version number. */
	dpy->dtype = rep.param.s[3];	/* Server's display type. */
	dpy->dplanes = rep.param.s[4];	/* Number of display bit planes. */
	dpy->dcells = rep.param.u[5];	/* Number of display color map cell. */
