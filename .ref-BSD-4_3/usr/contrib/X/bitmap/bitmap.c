...
#ifndef lint
static char *rcsid_bitmap_c = "$Header: bitmap.c,v 10.7 86/02/01 15:18:05 tony Rel $";
#endif
...
SetUp (argc, argv)
  ...
  if (file)
    ...
    if (dimensions != NULL) {
      if (sscanf (dimensions, "%2dx%2d", &squares_wide, &squares_high) != 2) {
  ...
  if (!XOpenDisplay(host)) {
    fprintf (stderr, "%s: could not connect to X server on %s\n",
    	progname, host);
    exit (1);
    }
...
DimensionsFromFile(file) 
  ...
  char variable[81];
  int value;
  while (fscanf (file, "#define %80s %2d\n", variable, &value) == 2) {
...
LayoutStage1 ()
  ...
  commands[8].name = "Set HotSpot";
  commands[8].proc = SetHotSpot;

  commands[9].name = "Clear HotSpot";
  commands[9].proc = ClearHotSpot;

  commands[10].name = "Write Output";
  commands[10].proc = WriteOutput;

  commands[11].name = "Quit";
  commands[11].proc = Quit;
