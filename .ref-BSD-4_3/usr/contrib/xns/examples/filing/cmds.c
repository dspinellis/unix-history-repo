#ifndef lint
static char RCSid[] = "$Header: cmds.c,v 2.0 85/11/21 07:22:41 jqj Exp $";
#endif
/* $Log:	cmds.c,v $
	...
	char *argv[];
{
	...
	if (argc > 2) {
		printf("usage: %s fileservice-name\n", argv[0]);
		return;
	}
	connected = hookup(argv[1]);
	if (connected != (CourierConnection*)0) {
		if (autologin) {
			char *pwd, name[128], *getXNSpass();
			printf("XNS UserName: ");
			gets(name);
	...
	char **argv;
{
	...
	if (argc < 3 )
		argv[2] = getXNSpass("Password: "), argc++;
	login(argv[1], argv[2]);
	return (1);
}
