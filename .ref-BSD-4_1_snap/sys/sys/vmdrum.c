	...
	struct text *xp;
{
	...
	if (xp->x_flag & XPAGI) {
		xp->x_ptdaddr = rmalloc(swapmap, clrnd(ctopt(xp->x_size)));
		if (xp->x_ptdaddr == 0) {
	...
	int ts;
{
	...
	if ((xp->x_flag&XPAGI) && xp->x_ptdaddr) {
		rmfree(swapmap, clrnd(ctopt(xp->x_size)), xp->x_ptdaddr);
		xp->x_ptdaddr = 0;
	}
}
