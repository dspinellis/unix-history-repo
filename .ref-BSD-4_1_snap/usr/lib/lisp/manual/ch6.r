...
This chapter describes  the  functions  which  one  uses  to
interact with FRANZ LISP running in the UNIX environment.
...
(allocate 's_type 'x_pages)
     ...
     SIDE EFFECT: FRANZ LISP attempts to allocate x_pages of
                  type  s_type.  It allocates pages one at a
                  time so that if an error occurs this means
                  that  all free storage has been allocated.
                  The storage that is allocated is not given
                  to  the caller, instead it is added to the
                  free storage list of  s_type.   The  func-
                  tions  _s_e_g_m_e_n_t  and _s_m_a_l_l-_s_e_g_m_e_n_t allocate
                  blocks  of storage and return  it  to  the
                  caller.
...
(argv 'x_argnumb)
     ...
     NOTE: if x_argnumb is less that zero,  a  fixnum  whose
...
(eval-when l_time g_exp1 ...)
     ...
     SIDE EFFECT: l_time may contain any combination of  the
                  ...
                  effects of load and compile will  is  dis-
                  cussed in the section on the compiler.  If
                  eval is present however, this simply means
                  that  the expressions g_exp1 and so on are
...
(gc)
     ...
     NOTE: garbage collection occurs automatically  whenever
           internal free lists are exhausted.
...
(process s_pgrm [s_frompipe s_topipe])
     ...
     SIDE EFFECT: If s_frompipe and s_topipe are given, they
                  ...
                  process respectively.  this forks  a  pro-
                  cess  named s_prgm and waits for it to die
                  iff there are no pipe arguments given.
...
(ptime)
     ...
     NOTE: the first number includes the second number.  The
...
(sstatus appendmap g_val)
     ...
     SIDE EFFECT: If g_val is non nil then when _f_a_s_l is told
                  ...
                  rather than creating a new map file.
...
(sstatus dumpcore g_val)
     ...
     NOTE: The default value for this flag is nil, and  only
...
(sstatus uctolc g_val)
     ...
     NOTE: This allows FRANZ LISP to be compatible with sin-
           gle case lisp systems (e.g. MAClisp).
...
(status localtime)
     ...
     RETURNS: a list of  fixnums  representing  the  current
              time  as  described  in  the UNIX manual under
              LOCALTIME(3).

     EXAMPLE: (_s_t_a_t_u_s _l_o_c_a_l_t_i_m_e) ==>  (20 51 16 29 5 80 0  1
              nil)


(status syntax s_char)

     RETURNS: a fixnum which is the syntax  code  associated
              with  the  character  s_char  in  the  current
              readtable.

     NOTE: You  cannot  set  the  syntax  code   with   with
           (_s_s_t_a_t_u_s _s_y_n_t_a_x _c_h), you must use _s_e_t_s_y_n_t_a_x.
