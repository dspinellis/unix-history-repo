   ...
   _8._1.  _v_a_l_i_d _f_u_n_c_t_i_o_n _o_b_j_e_c_t_s
      ...
      the  function field of a symbol object.  The following
      table shows all of the possibilities, how to recognize
...
7777777777777777777
   ...
   _8._2.  _f_u_n_c_t_i_o_n_s   The basic lisp function is  the  lambda
      function.  When  a  lambda  function  is  called,  the
      actual  arguments are evaluated from left to right and
      are lambda-bound  to  the  formal  parameters  of  the
      lambda function.
      ...
      thing  to  do  as it won't work compiled if any of the
      arguments are local variables. The solution is to  use
      a  lexpr.   When a lexpr function is called, the argu-
      ments are evaluated and the  number  of  arguments  is
      lambda-bound  to  the  single  formal parameter of the
      lexpr function.  The lexpr then accesses the arguments
      using the _a_r_g function.

           When a function is compiled  _s_p_e_c_i_a_l  declaration
      ...
      declared _s_p_e_c_i_a_l (see  12.3.2.2).   Lambda  and  lexpr
      functions  both  compile  into  a binary object with a
      discipline of lambda.  However, a compiled lexpr still
      acts like an interpreted lexpr.




   _8._3.  _m_a_c_r_o_s   An important features of Lisp is its abil-
      ity  to  manipulate  programs as data.  As a result of
      this, most Lisp  implementations  have  very  powerful
      macro  facilities.  The Lisp language's macro facility
...
Functions and Macros                                     8-3
      ...
      can be used to incorporate  popular  features  of  the
      other  languages  into  Lisp.   For example, there are
      macro packages which allow one to create records  (ala
      Pascal)  and refer to elements of those records by the
      key names.[] Another  popular use  for  macros  is  to
      create  more  readable control structures which expand
      into _c_o_n_d, _o_r and _a_n_d.  One such  example  is  the  If
      macro  in  the  jkfmacs.l  package.   It allows you to
      write
      ...
      _8._3._1.  _m_a_c_r_o _f_o_r_m_s   A  macro  is  a  function  which
         accepts  a  Lisp  expression  as  input and returns
         another Lisp  expression.   The  action  the  macro
         takes  is called macro expansion.  Here is a simple
         example:
         ...
         evaluates _f_i_r_s_t's definition passing to _f_i_r_s_t as an
         argument, the form _e_v_a_l itself was trying to evalu-
         ate,  (_f_i_r_s_t '(_a _b _c)).   The _f_i_r_s_t macro chops off
         the car of the argument with _c_d_r, cons'  a  _c_a_r  at
____________________
...
Functions and Macros                                     8-4
         ...
         the   beginning   of   the   list    and    returns
         (_c_a_r '(_a _b _c)).   Now  _e_v_a_l evaluates that, and the
         value is _a  which  is  returned  as  the  value  of
         (_f_i_r_s_t '(_a _b _c)).   Thus  whenever  _e_v_a_l  tries  to
         evaluate a list whose car has a macro definition it
         ends  up  doing (at least) two operations, one is a
         call to the macro to let it macro expand the  form,
         and  the  other  is the evaluation of the result of
         the macro.  The result of  the  macro  may  be  yet
         another  call  to  a  macro, so _e_v_a_l may have to do
         even more evaluations until it can  finally  deter-
         mine  the   value of an expression.  One way to see
         how a macro will expand is to use _a_p_p_l_y as shown on
         the third input line above.




      _8._3._2.  _d_e_f_m_a_c_r_o   The macro _d_e_f_m_a_c_r_o makes it  easier
         to  define macros because it allows you to name the
         arguments to the macro call.  For example,  suppose
         we   find   ourselves   often   writing  code  like
      ...
      _8._3._3.   _t_h_e _b_a_c_k_q_u_o_t_e _c_h_a_r_a_c_t_e_r _m_a_c_r_o    The  default
         syntax  for  FRANZ  LISP  has only three characters
         with associated character macros.  One is semicolon
         for  comments.   The  other  two  are backquote and
         comma which are used  by  the  backquote  character
         macro.  The backquote macro is used to create lists
...
Functions and Macros                                     8-5
   ...
   _8._4.  _f_o_r_e_i_g_n _s_u_b_r_o_u_t_i_n_e_s _a_n_d _f_u_n_c_t_i_o_n_s   FRANZ LISP  has
      the  ability to dynamically load object files produced
      by other compilers and then call functions defined  in
      those files.  These functions are called _f_o_r_e_i_g_n func-
      tions.  There are four types of foreign functions  and
      they  are  characterized  by  the  type of result they
      return:
...
Functions and Macros                                     8-6
      ...
      that the discipline field for a  binary  object  of  a
      foreign  function is a string whose first character is
      s for a  subroutine,  f  for  a  function,  i  for  an
      integer-function and r for a real-function.  Two func-
      tions are provided for the setting up of foreign func-
      tions.   _C_f_a_s_l loads an object file into the lisp sys-
      tem and sets up one foreign  function  binary  object.
      If  there is more than one function in an object file,
      _g_e_t_a_d_d_r_e_s_s can be used to set up further foreign func-
      tion objects.

           Foreign  functions are  called  just  like  other
      functions,   e.g  (_f_u_n_n_a_m_e _a_r_g_1 _a_r_g_2).   When  one  is
      called, the arguments are evaluated and then examined.
      List,  hunk  and symbol arguments are passed unchanged
      to the foreign function.  Fixnum and flonum  arguments
      are  copied into a temporary location and a pointer to
      the value is passed (this is because Fortran uses call
      by  reference  and  it is dangerous to modify the con-
      tents of a fixnum or flonum which something else might
      point to).  If an array object is an argument the data
      field of the array  object is passed  to  the  foreign
      function  (this  is  the  easiest  way  to  send large
      amounts of data to and receive large amounts  of  data
      from  a  foreign  function).  If a binary object is an
      argument, the entry field of that object is passed  to
      the  foreign  function (the entry field is the address
      of a function, so this amounts to passing  a  function
      as an argument).
...
Functions and Macros                                     8-7
      ...
      The first (cfoo is C, pfoo in Pascal)  is  given  four
      ...
      _b_o_l_d,  computer output is in roman and comments are in
      _i_t_a_l_i_c.
...
Functions and Macros                                     8-8
      ...
      % lisp
      Franz Lisp, Opus 33b
      _F_i_r_s_t _t_h_e _f_i_l_e_s _a_r_e _l_o_a_d_e_d _a_n_d _w_e _s_e_t _u_p  _o_n_e  _f_o_r_e_i_g_n
      _f_u_n_c_t_i_o_n  _b_i_n_a_r_y.   _W_e _h_a_v_e _t_w_o _f_u_n_c_t_i_o_n_s _i_n _e_a_c_h _f_i_l_e
      _s_o _w_e _m_u_s_t _c_h_o_o_s_e _o_n_e _t_o _t_e_l_l _c_f_a_s_l _a_b_o_u_t.  _T_h_e _c_h_o_i_c_e
      _i_s _a_r_b_i_t_r_a_r_y.
...
Functions and Macros                                     8-9
      ...
      #6306c-"function"
      _W_e _w_a_n_t _t_o _c_r_e_a_t_e _a_n_d _i_n_i_t_i_a_l_i_z_e _a_n _a_r_r_a_y _t_o  _p_a_s_s  _t_o
      _t_h_e  _c_f_o_o _f_u_n_c_t_i_o_n.  _I_n _t_h_i_s _c_a_s_e _w_e _c_r_e_a_t_e _a_n _u_n_n_a_m_e_d
      _a_r_r_a_y _a_n_d _s_t_o_r_e _i_t _i_n _t_h_e _v_a_l_u_e _c_e_l_l _o_f _t_e_s_t_a_r_r.  _W_h_e_n
      _w_e  _c_r_e_a_t_e  _a_n  _a_r_r_a_y _t_o _p_a_s_s _t_o _t_h_e _P_a_s_c_a_l _p_r_o_g_r_a_m _w_e
      _w_i_l_l _u_s_e_d _a _n_a_m_e_d _a_r_r_a_y _j_u_s_t _t_o _d_e_m_o_n_s_t_r_a_t_e  _t_h_e  _d_i_f_-
      _f_e_r_e_n_t  _w_a_y  _t_h_a_t _n_a_m_e_d _a_n_d _u_n_n_a_m_e_d _a_r_r_a_y_s _a_r_e _c_r_e_a_t_e_d
      _a_n_d _a_c_c_e_s_s_e_d.
      ...
      a: 385, b[0]: 1.234000, b[1]: 5.678000
       ...
       ( 15 16 ... )
       3
      _N_o_t_e _t_h_a_t _c_f_o_o _h_a_s _r_e_t_u_r_n_e_d _3 _a_s _i_t _s_h_o_u_l_d.   _I_t  _a_l_s_o
      _h_a_d  _t_h_e  _s_i_d_e  _e_f_f_e_c_t _o_f _c_h_a_n_g_i_n_g _t_h_e _s_e_c_o_n_d _v_a_l_u_e _o_f
      _t_h_e _a_r_r_a_y _t_o _3._1_4_1_5_9_2_6  _w_h_i_c_h _c_h_e_c_k _n_e_x_t.
      -> (funcall testarr 1)
      3.1415926
