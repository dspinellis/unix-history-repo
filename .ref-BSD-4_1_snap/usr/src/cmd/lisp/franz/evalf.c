static char *sccsid = "@(#)evalf.c	35.2 7/1/81";
...
Levalf ()
{
	...
	verifypdlp(myfp);	/* make sure we are given valid pointer */
...
/*
 * verifypdlp :: verify pdl pointer as existing,  do not return unless
 * it is valid
 */
verifypdlp(curfp)
register struct frame *curfp;
{
    ...
    errorh(Vermisc,"Invalid pdl pointer given: ",nil,FALSE,0,inewint(curfp));
}
...
Lfretn ()
{
    ...
    verifypdlp(myfp);		/* make sure pdlp is valid */
